...
const new{{.Name}} = () => {
    return { {{range .Fields}}{{if not .Nullable}}{{if eq .Type "string"}}
        {{c .Name}}: "",{{else if eq .Type "bool"}}
        {{c .Name}}: false,{{else if or (eq .Type "int") (eq .Type "float")}}
        {{c .Name}}: 0,{{else if or (eq .Type "int[]") (eq .Type "float[]") (eq .Type "string[]") (eq .Type "id[]")}}
        {{c .Name}}: [],{{else if or (eq .Type "map[string]int") (eq .Type "map[string]float") (eq .Type "map[string]string") (eq .Type "map[string]string[]")}}
        {{c .Name}}: {},{{else}}
        {{c .Name}}: null,{{end}}{{end}}{{end}}
    }
}

// 映射关联字段
const mapData = (result: any) => { {{range .Fields}}{{if eq .Name "Password"}}
    result.data.forEach((i: any) => i.password = null);{{else if .Link}}
    /* 处理外键-{{.Description}} */{{if eq .Type "id[]"}}
...
// 反映射关联字段
const reverseMap = (model: any) => { {{range .Fields}}{{if .Link}}
    // {{.Description}} {{if eq .Type "id"}}
    if (model.{{c .Name}}) model.{{c .Name}} = model.{{c .Name}}.id ? model.{{c .Name}}.id : model.{{c .Name}};{{else if eq .Type "id[]"}}
    if (model.{{c .Name}}) {
        let ids: string[] = [];
...
export default (props: TableProps<{{.Name}}>) => {
    ...
    const { loading } = useRequest({
        ...
        method: 'post',
        data: { where: { trash: trash, ...reverseMap(exchangeNullable(where)), ...props.where }, sort: sort, pagination: pagination },{{if .Link}}
        headers: {
    ...
    const columnButtons = (model: {{.Name}}): { [key: string]: JSX.Element } => {
        return {
            'update': <Tooltip key="upload" title="修改"><EditOutlined onClick={e => {
                ...
                } }/></Tooltip>,{{if .Upload}}{{range .Fields}}{{if .Upload}}
            'upload{{.Name}}': <FileUpload key="upload{{.Name}}"
                ...
                <Tooltip title="上传{{.Description}}"><UploadOutlined /></Tooltip></FileUpload>,{{end}}{{end}}{{end}}
        }
    }
    // 表格按钮
